<?php
/**
 * PixelPop\Styles\Component class
 *
 * @package pixelpop
 */

namespace PixelPop\Styles;
...
use function PixelPop\pixelpop_is_primary_sidebar_active;

/**
 * Class for managing stylesheets.
 *
 * Exposes template tags:
 * * `pixelpop()->print_styles()`
 */
class Component implements Component_Interface, Templating_Component_Interface {
	...
	public function initialize() {
		...
		add_action( 'customize_controls_print_styles', array( $this, 'pixelpop_customizer_styles' ) );
		add_action( 'admin_init', array( $this, 'pixelpop_fontawesome_dashboard' ) );
	}
	...
	public function action_enqueue_styles() {
		...
		if ( ! empty( $google_fonts_url ) ) {
			wp_enqueue_style( 'pixelpop-fonts', $google_fonts_url, array(), null ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
		}
		...
		foreach ( $css_files as $handle => $data ) {
			...
			$version = pixelpop()->get_asset_version( $css_dir . $data['file'] );
	...
	public function filter_resource_hints( array $urls, string $relation_type ) : array {
		if ( 'preconnect' === $relation_type && wp_style_is( 'pixelpop-fonts', 'queue' ) ) {
			$urls[] = array(
		...
		$handles   = array_filter(
			...
			function( $handle ) use ( $css_files ) {
				...
				if ( ! $is_valid ) {
					...
					_doing_it_wrong( __CLASS__ . '::print_styles()', esc_html( sprintf( __( 'Invalid theme stylesheet handle: %s', 'pixelpop' ), $handle ) ), 'PixelPop 2.0.0' );
				}
				return $is_valid;
			}
		);
	 ...
	 * {@see 'pixelpop_preloading_styles_enabled'} filter can be used to tweak the return value.
	 *
	...
	protected function preloading_styles_enabled() {
		$preloading_styles_enabled = ! pixelpop()->is_amp();
		 ...
		 */
		return apply_filters( 'pixelpop_preloading_styles_enabled', $preloading_styles_enabled );
	}
	...
	protected function get_css_files() : array {
		...
		$css_files = array(
			'pixelpop-global' => array(
				'file'   => 'global.min.css',
				'global' => true,
			),
			'pixelpop-comments' => array(
				'file'             => 'comments.min.css',
				'preload_callback' => function() {
					return ! post_password_required() && is_singular() && ( comments_open() || get_comments_number() );
				},
			),
			'pixelpop-content' => array(
				'file'             => 'content.min.css',
				'preload_callback' => '__return_true',
			),
			'pixelpop-sidebar' => array(
				'file'             => 'sidebar.min.css',
				'preload_callback' => function() {
					return pixelpop()->is_primary_sidebar_active() && pixelpop_is_primary_sidebar_active();
				},
			),
			'pixelpop-widgets' => array(
				'file'             => 'widgets.min.css',
				'preload_callback' => function() {
					return pixelpop()->is_primary_sidebar_active() && pixelpop_is_primary_sidebar_active();
				},
			),
			'pixelpop-front-page' => array(
		 ...
		 */
		$css_files = apply_filters( 'pixelpop_css_files', $css_files );
	...
	protected function get_google_fonts() : array {
		...
		$google_fonts = array(
			'Nunito'       => array( '200', '200i', '300', '300i', '400', '400i', '600', '600i', '700', '700i', '800', '800i', '900', '900i' ),
			'Nunito Sans'  => array( '200', '200i', '300', '300i', '400', '400i', '600', '600i', '700', '700i', '800', '800i', '900', '900i' ),
		);
		 ...
		 */
		$this->google_fonts = (array) apply_filters( 'pixelpop_google_fonts', $google_fonts );
	 ...
	 * Customizer CSS.
	 */
	public function pixelpop_customizer_styles() {

		wp_enqueue_style( 'pixelpop-customizer-css', get_template_directory_uri() . '/assets/css/customizer.min.css', false, '1.0.0' );

	}

	/**
	 * Enqueue Font Awsome to Admin Dashboard.
	 */
	public function pixelpop_fontawesome_dashboard() {
		wp_enqueue_style( 'font-awesome-free', get_stylesheet_directory_uri() . '/assets/fonts/font-awesome/css/all.css', false, '1.0.0' );
		wp_enqueue_style( 'pixelpop-admin-css', get_template_directory_uri() . '/assets/css/admin.min.css', false, '1.0.0' );
		wp_enqueue_style( 'pixelpop_google_fonts', 'https://fonts.googleapis.com/css2?family=Nunito:ital,wght@0,200;0,300;0,400;0,600;0,700;0,800;0,900;1,200;1,300;1,400;1,600;1,700;1,800;1,900&display=swap', false, '1.0.0' );
		wp_enqueue_style( 'ppop-feather-icons', get_stylesheet_directory_uri() . '/assets/fonts/feather/feather-icons.css', false, '1.0.0' );
	}
}
